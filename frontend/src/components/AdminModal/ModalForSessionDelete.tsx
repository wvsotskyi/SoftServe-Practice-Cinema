...
interface ModalForSessionDeleteProps {
  isOpen: boolean;
  onConfirm: () => void;
  onCancel: () => void;
}

const ModalForSessionDelete: React.FC<ModalForSessionDeleteProps> = ({
  isOpen,
  onConfirm,
  onCancel,
}) => {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60 px-4">
      <div className="bg-[#1C1B20] text-white rounded-2xl shadow-2xl max-w-md w-full p-6 sm:p-8">
        <h2 className="text-2xl font-semibold text-center mb-6">
          Ви дійсно хочете видалити цей сеанс?
        </h2>
...
export default ModalForSessionDelete;
